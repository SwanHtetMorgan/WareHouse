...
public class TokenSerivceImpl implements TokenServices, AuthorizationServices {
  ...
  @SneakyThrows
  @Override
  public byte[] encryption(String userInfo)
  ...
  /**
   * @Author : Swan Htet Aung Phyo @Usage : For the authorization Checking
   */
  @Override
  public boolean dbInfoMatcher(List<String> segmentation) {
    ...
    Duration duration = Duration.between(expiredAt, LocalDateTime.now());
    if (duration.toMinutes() < 10) {
      expiredAt = expiredAt.plusHours(24);
    }
    return expiredAt.isAfter(LocalDateTime.now());
  }

  @Override
  public String RoleExtraction(List<String> Segmentation) {
    return Segmentation.get(1);
  }
}
